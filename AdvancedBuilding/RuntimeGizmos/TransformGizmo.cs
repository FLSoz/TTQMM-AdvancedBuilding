...
namespace RuntimeGizmos
{
    ...
    public class TransformGizmo : MonoBehaviour
    {
        ...
        public bool circularRotationMethod = true;
        ...
        public List<Transform> TargetRootsOrdered { get { return targetRootsOrdered; } }
        ...
        /*void SetSpaceAndType()
        {
            ...
            else if (Input.GetKeyDown(SetRectToolType)) transformType = TransformType.RectTool;
        ...
        void TransformSelected()
        {
            ...
            {
                if (GUIUtility.hotControl == 0 && nearAxis != Axis.None && Input.GetMouseButtonDown(0))
                {
                    ...
                    if (transType == TransformType.Move)
                    {
                        ...
                        if (isSnapping && movementSnap > 0)
                        {
                            ...
                            if (hasTranslatingAxisPlane)
                            {
                                ...
                                float snapAmount1 = CalculateSnapAmount(movementSnap, amountInAxis1, out float remainder1);
                                float snapAmount2 = CalculateSnapAmount(movementSnap, amountInAxis2, out float remainder2);
        ...
        void GetTarget()
        {
            if (nearAxis == Axis.None && Input.GetMouseButtonDown(0))
            {
                ...
                var vis = Singleton.Manager<ManPointer>.inst.targetVisible;
                //RaycastHit hitInfo;
                if (/*Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask)*/vis && vis.block)
                {
                    Transform target = vis.block.trans;//hitInfo.transform;

                    if(targetRootsOrdered.Count > 0)
                    {
                        if(targetRootsOrdered.Last().GetComponent<TankBlock>().tank != vis.block.tank)
                        {
                            return;
                        }
                    }
                    ...
                    if (isAdding)
                    {
                        if(targetRootsOrdered.Contains(target))
                        {
                            RemoveTarget(target);
                        }
                        else
        ...
        void AddTargetHighlightedRenderers(Transform target)
        {
            if (target != null)
            {
                ...
                renderersBuffer = renderersBuffer.Where(r => !r.gameObject.name.Contains("ObjectHighlight")).ToList();

                for (int i = 0; i < renderersBuffer.Count; i++)
                {
                    Renderer render = renderersBuffer[i];

                    var block = render.gameObject.GetComponentInParent<TankBlock>();

                    if(block)
                    {
                        block.visible.EnableOutlineGlow(true, cakeslice.Outline.OutlineEnableReason.ScriptHighlight);
                    }                  
                    else if (!highlightedRenderers.Contains(render))
                    {
        ...
        void SetMaterial()
        {
            ...
            {
                lineMaterial = new Material(AdvancedBuildingMod.assetBundle.LoadAsset<Shader>("Lines"));
                outlineMaterial = new Material(AdvancedBuildingMod.assetBundle.LoadAsset<Shader>("Outline"));
            }
        }
    }
}
